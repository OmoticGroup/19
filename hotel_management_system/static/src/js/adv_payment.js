...
publicWidget.registry.WebsiteOrderAdvancePayments = publicWidget.Widget.extend({
        ...
        if (amount >= parseFloat(this.miniAdvanceAmount) && amount <= maxAmount) {
            ...
            iFrame.onload = function () {
                if (iFrame.contentDocument.body.querySelector('button[name="o_payment_submit_button"]')) {
                    ...
                    // iFrame.contentDocument.body.querySelector('button[name="o_payment_submit_button"]').classList.remove('d-none');
                    // iFrame.contentDocument.body.querySelector('button[name="o_payment_submit_button"]').removeAttribute('disabled');
                    const paymentScript = document.createElement('script');
                    paymentScript.textContent = `
                        document.querySelectorAll('input[name="o_payment_radio"]').forEach(radio => {
                            radio.addEventListener('change', function() {
                                const inlineForms = document.querySelectorAll('[name="o_payment_inline_form"]');
                                inlineForms.forEach(form => form.classList.add('d-none'));
                                
                                const selectedForm = this.closest('[name="o_payment_option"]')?.querySelector('[name="o_payment_inline_form"]');
                                if (selectedForm) {
                        ...
                        if (submitBtn) {
                            submitBtn.addEventListener('click', function(e) {
                                ...
                                window.parent.postMessage({type: 'PAYMENT_SUBMIT', data: 'payment_data'}, '*');
                            });
                        }
                    `;
                    iFrame.contentDocument.head.appendChild(paymentScript);
                }
                else {
    ...
    _checkHotelAdvanceConfig: async function () {
        ...
        `;
            document.querySelector('.advance-payment-note')?.remove();
